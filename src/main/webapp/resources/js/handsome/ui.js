(function($) {
	/*
	 * $(document).ready(function(){ $(window).scroll(function(){ if
	 * ($(this).scrollTop() >= 1) { $('.header').addClass('header_scroll');
	 * $('#bodyWrap').css('margin-top','120px'); } else {
	...
	$(document).ready(
			function() {
				...
				$('.list_item4 > ul > li').each(
						function(list_item4) {
							if ((list_item4 % 4) == 0) {
								$(
										'.list_item4 > ul > li:nth-child('
												+ (list_item4 + 4) + ')')
										.addClass('mr0');
							}
						});
				...
				$('.list_item3 > ul > li').each(
						function(list_item3) {
							if ((list_item3 % 3) == 0) {
								$(
										'.list_item3 > ul > li:nth-child('
												+ (list_item3 + 3) + ')')
										.addClass('mr0');
							}
						});
	...
	$(function() {
		$(".zipcode .result_box .list li:nth-child(even)").css("background",
				"#f8f8f8");
	...
	$(document).ready(
			function() {
				var $br_margin_left = ($("h1").width() + $(".brd_m").width()
						+ $(".cate_m > li:first-child").width() + 20);
				var $br_margin_right = (1205 - ($("h1").width()
						+ $(".gnb_nav").width() + 50));
				$(".gnb_nav > ul.cate_m.cate_m_br li div ul").each(
						function(index) {
							$(this).css('margin-left', $br_margin_left);
						});
				$(".gnb_nav > ul.cate_m.cate_m_br li+li+li+li+li div ul").each(
						function(index) {
							$(this).css('margin-right', $br_margin_right).css(
									'margin-left', 0).css('float', 'right');
						});
			});
	...
	$(document)
			.ready(
					function() {
						...
						$('.brand_menu1803 .plus_brand').css({
							left : brTopMenu,
							display : 'block'
						});
						var brTopMenu2 = $('.brand_menu1903 > ul').width();
						$('.brand_menu1903 .plus_brand').css({
							left : brTopMenu2,
							display : 'block'
						});
						var $cate_m = $(".gnb_nav > ul.cate_m > li");
						$cate_m.find('.sub_menu').children('ul').addClass(
								'sm_dep1');
						$cate_m.find('.sub_menu').children('ul').children('li')
								.children('ul').addClass('sm_dep2');
						$cate_m.find('.sub_menu').children('ul').children('li')
								.children('ul').children('li').children('ul')
								.addClass('sm_dep3');

						$(".gnb_nav > ul.cate_m > li")
								.each(
										function(index) {
											var $sub_back = $(this).children(
													".sub_back");
											var $sub_menu = $sub_back
													.children(".sub_menu");
											var $sm_dep1 = $sub_menu
													.children(".sm_dep1");
											var $sm_dep2 = $sm_dep1.children()
													.children(".sm_dep2");
											var $sm_dep3 = $sm_dep2.children()
													.children(".sm_dep3");
											var $sm_height = $sub_menu
													.outerHeight();
						...
						// Top Menu
						$(".header_dropmemu").mouseover(
								function() {
									$(this).children(".btn").addClass("on");
									$(this).children("span").siblings("a")
											.addClass("on");
						...
						$(".header_dropmemu > .list").mouseover(
								function() {
									$(this).siblings("span").children("a")
											.addClass("on");
								});
						$(".header_dropmemu > .list").mouseleave(
								function() {
									$(this).siblings("span").children("a")
											.removeClass("on");
								});
	...
	$(document).ready(
			function() {
				$(".tab_a li a, .tab_d li a").click(
						function() {
							$(this).addClass("active");
							$(this).parent("li").siblings("li").children("a")
									.removeClass("active");
						});

				$(".tab_b li a, .tab_c li a").click(
						function() {
							$(this).addClass("on");
							$(this).parent("li").siblings("li").children("a")
									.removeClass("on");
				...
				$(".gnb_sh_box > a").click(
						function() {
							$(this).parents().siblings(".gnb_sh_result").css(
									"display", "block");
						});
				// Search Top10
	...
	$(document).ready(
			function() {
				$(document).on(
						...
						'.btn_option, .add_bag',
						function() {
							$(this).parents("tr").next("tr").find(
									".basket_info").stop().slideDown('fast');
							$(this).parents("tr").next("tr").next('tr').find(
									".store_info1807").stop().slideUp('fast');
						});
				...
				$(document)
						.on(
								"click",
								'.btn_store_option1807',
								function() {
									$(this).parents("tr").next("tr").next('tr')
											.find(".store_info1807").stop()
											.slideDown('fast');
									$(this).parents("tr").next("tr").find(
											".basket_info").stop().slideUp(
											'fast');
								});

				$(document).on(
						"click",
						".store_info1807 .btn_close",
						function() {
							$(this).parents('.store_info1807').stop().slideUp(
									'fast');
						});
			});
	...
	$(document).ready(
			function() {
				...
				$(document).on(
						"click",
						'.color_chip li a, .size_chip li a',
						function() {
							$(this).addClass("on");
							$(this).parent("li").siblings("li").children("a")
									.removeClass("on");
				...
				$(document).on(
						"mouseover",
						".st_receipt_wrap div .tlt",
						function() {
							$(this).parent('div').find('.delch_box').css(
									"display", "block");
						});
				$(document).on(
						"mouseout",
	...
	$(document).ready(
			function() {
				$('.related_site > dl').click(
						function(e) {
							$(this).find('dd').slideToggle('fast'), $(this)
									.find('dt > a').toggleClass('on'), e
									.stopPropagation();
						});
	...
	$(document)
			.ready(
					function() {
						/* 211109 공동현관 출입방법 S */
						$('.input_sumtxt_box:not(.input_sumtxt_box2)').click(
								function(e) {
									$('.delivery_comt:not(.delivery_comt2)')
											.toggle();
									$('.delivery_comt2').hide();
									e.stopPropagation();
								});
						...
						$('.input_sumtxt_box2').click(function(e) {
							$('.delivery_comt2').toggle(), e.stopPropagation();
						});
								/* 211109 공동현관 출입방법 E */
								$("body").click(function() {
									$('.delivery_comt').hide();
								}),
								$(".input_sumtxt_box input").keydown(
										function() {
											$('.delivery_comt').hide();
										}),
								$(".delivery_comt li a")
	...
	$(document).ready(
			function() {
				...
				$(".filterWrap > ul > li > a").click(
						function() {
							var $this = $(this), $list = $this.next(".list");

							// 화면이 출력되는 경우 처리
							if ($list.is(':visible') === false) {
								$list.find('li').unbind('click.select').one(
										'click.select',
										function() {
											var $currentList = $(this).parents(
													'ul div.list');
											$currentList.slideUp('fast');
											// 기존에 있던 이벤트를 삭제한다.
											$currentList.find('li').unbind(
													'click.select');
										})
							}
	...
	$(document).ready(
			function() {
				$(document)
						.on(
								"mouseover",
								".tooltip191022 .ico_question",
								function() {
									$(this).parent('.tooltip191022').find(
											'.delch_box190816').css("display",
											"block");
								});
				$(document).on(
						"mouseout",
						...
						function() {
							$(this).parent('.tooltip191022').find(
									'.delch_box190816').css("display", "none");
						});
			});
	function instaImage() {
