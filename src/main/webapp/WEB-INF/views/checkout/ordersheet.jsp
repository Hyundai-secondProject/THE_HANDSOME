...
<script type="text/javascript">
		...
		var ACC = { config: {} };
			...
			ACC.config.CSRFToken = "53ca9b9c-c912-4cba-92cb-e959f1b4e2e5";
...
function callChatbot(token, talkId) {
    if(token == null) {
        $.ajax({
            ...
            data: {"CSRFToken":"53ca9b9c-c912-4cba-92cb-e959f1b4e2e5"},
            async : false,
            success:function(data){
...
$(document).ready(function() {
    ...
    if("PASS" != ""){
	    ...
	    if ((vipPassCheck == "Y" || timePassCheck == "Y") && "ordersheet" == "main201903Page") {
	        $('#chatbotMain').show(); 
	    }else{
	        $('#chatbotMain').hide(); 
	    }
    ...
    */
    ...
    $("#confirm_btn_selectComp").click(function(){
        ...
        if($(":radio[id='deilvy_rdo01']").prop("checked")){
            ...
            if( hasOeraEntry == 'true' || hasLiquidesPerfums == 'true'){
                ...
                $(".oera_fkh_003-case4").html("※ THE STAR/STAR/MANIA 대상 선택사항입니다<br/>※ 우체국 택배는 토요일 출고 및 배송 불가");
            }
    ...
    if(parseInt(0) > 0){
     ...
     $('#hpoint_useall').click(function(){
         if($('#hpoint_useall').is(":checked") && parseInt(1204) > 0){
            var usehpoint = $('#total').val() - $('#cartDeliveryCost').val();
            if(parseInt(1204) > usehpoint ){
     ...
     $('#hpointpay').blur(function(){
         if(parseInt(1204) > 0 && $('#hpointpay').val() >= parseInt(1204)){
            $('#hpointpay').val('1204');
            if($('#hpointpay').val() > 0){
...
function doOrder(){
    ...
    if($('#commissionAgreeYN').is(":checked")){
   	 var commissionAgreeYN = "<input type=hidden name='commissionAgree' value='Y'>";
   	    $('#orderForm').append(commissionAgreeYN);
  	 }else{
   	var commissionAgreeYN = "<input type=hidden name='commissionAgree' value='N'>";
   		$('#orderForm').append(commissionAgreeYN);
   }
...
function doUseHPoint(self){
    ...
    if(use_point >= 100000 && parseInt(1204) >= 100000){
        $('#counter').html("03:00초");  
        $('#codeInput').val('');
	    ...
	    'checkout': {
		    ...
		    'products':
			    [
				    ...
				    "id": "SH2C7ASZ075M", //상품코드
				    "name": "레더 믹스드 스니커즈", //상품명
				    "brand": "SYSTEM HOMME", //상품 브랜드
				    
				    "category": decodeURIComponent("FASHION-ACC-")+"/"+decodeURIComponent("MEN-SHOES")+"/"+decodeURIComponent("SNEAKERS"), //상품 카테고리
				    "variant": "BEIGE_275", //변형 제품(옵션)
				    "quantity": "1", //상품 수량
