						...
						.ready(
								function() {
									...
									var voucherList = new BoardListController(
											'searchForm', '#listBody',
											'.paging', 20); //폼아이디, 리스트공간 셀렉터, 페이징공간 셀렉터, 페이지당 리스트개수
									...
									if (id != 'N') {
										$(".txt_line")
												.text(
														"쿠폰 사용 및 유효 기간 만료 후 6개월 간 내역입니다.");
									}
											...
											$
													.each(
															...
															function() {
																...
																listTag += '	<td class="frt" style="word-break:keep-all;">'
																		+ this.name
																		+ '</td>';
																listTag += '	<td>'
																		+ this.vouchercode
 ...
 -->
			<form id="searchForm" action="/ko/mypage/voucherOnline">
				<div class="title_wrap mt30 clearfix">
					<h4 class="float_left">
						<label for="id_sel">온라인 쿠폰 목록<!-- 온라인 쿠폰 목록 --></label>
					</h4>
					<p class="txt_line"></p>
					<div class="btn_wrap">
						...
						<sec:authentication property="principal.username" var="MID"/>
							<option value="/mypage/mycouponUnused?MID=${MID}" >미사용
								<!-- 미사용 --></option>
							<option value="/mypage/mycouponUsed?MID=${MID}">사용
								<!-- 사용 --></option>
							<option value="/mypage/mycouponExpiry?MID=${MID}" selected>기간만료
								<!-- 기간만료 --></option>
						</select>
					</div>
				</div>
							...
							<tr class="al_middle">
								...
								<td class="al_left">
									<ul class="bul_sty01_li">
										<li>${coupon.econtent}</li>
									</ul>
								</td>
								<td> ${coupon.ediscount} %</td>
								<td><fmt:formatDate value="${coupon.cpissuedate}"
										type="both" pattern="YYYY.MM.dd" /> ~ <fmt:formatDate
										value="${coupon.cpexpiredate}" type="both"
										pattern="YYYY.MM.dd" />
								<td><fmt:formatDate value="${coupon.cpusedate}" type="both"
			...
			<div class="gd_wrap mt50">
				<dl class="gd_list com pl0 svc_gd_list">
					...
					<dd>
						<ul class="bul_sty01_li">
							<li>온라인쿠폰 : 상품 주문 시 ‘주문&amp;배송정보입력’ 화면에서 쿠폰을 적용하면 할인된 금액으로
								상품을 구입하실 수 있습니다.</li>
			...
			<div class="gd_wrap">
				<dl class="gd_list com pl0 svc_gd_list">
					...
					<dd>
						<ul class="bul_sty01_li">
							<li>생일축하 한섬마일리지 : 생일을 맞은 회원님께 생일축하 2만 한섬마일리지를 드립니다.(온/오프라인
								합하여, 구매이력 있는 고객)</li>
							<li>신규회원 온라인 할인쿠폰 : 한섬 온라인에 가입한 즉시 발급되며, 1개월 이내 사용 하셔야 합니다.<br />할인
								금액의 제한은 없습니다.
							</li>
							<li>한섬 우수고객 혜택 (2019년) : 연간 구매금액 1,000만원 이상 회원 대상으로 다음 해 연간
								구매 혜택, 서비스, 기프트를 증정합니다.</li>
...
<script type="text/javascript">
											...
											function() {
												...
												if ($(this).attr("id").indexOf(
													...
													if (liLen < 5) {
														...
														$(
																'form[name=reviewForm] .upload_btn label')
																.eq(fileLen - 1)
																.hide();
														$(
																'form[name=reviewForm] .upload_btn input[type=file]')
																.eq(fileLen - 1)
																.hide();
														$(
																"form[name=reviewForm] .upload_btn")
																.append(html);
														...
														html += '<span>'
																+ $(this)[0].files[0].name
																+ '</span>';
														html += '<a href="javascript:void(0);" class="file_del" onclick="deleteReviewFile($(this),'
																+ fileIdx
																+ ');">파일삭제</a>';
														html += '</li>';
														$(
																"form[name=reviewForm] .file_image_wrap2003")
																.append(html);
	...
	function viewPopup(v) {
		...
		obj.css("top", tp).css("left", lp).css("position", "absolute").css(
				"z-index", 101);
		obj.show();
	...
	function starCnt() {//상품평 별점 선택
		$('.btn_star_score li a').on(
				...
				function() {
					...
					for (var i = 1; i < 6; i++) {
						$('.write_star_score1807 .cmt_star .cmt_per')
								.removeClass('star' + i);
					}
	...
	function reviewReset() {
		...
		$('#customerReviewWriteDiv .upload_btn label')
				.each(
						function(i) {
							if (i != labelLeng) {
								$('#customerReviewWriteDiv .upload_btn label')
										.eq(0).remove();
								$(
										'#customerReviewWriteDiv .upload_btn input[type=file]')
										.eq(0).remove();
							}
						});
	...
	function uploadFilesubmit(writeType, returnType) {
		...
		var fileSize = $('form[name=reviewForm] .upload_btn input[type=file]')
				.size();
		//사진 전체 삭제여부 체크
		if (liSize == 0) {
			$("#fileDeleteYN").val("Y");
		...
		if (fileSize <= 1) {
			...
			$
					.ajax({
						url : url
								+ '?CSRFToken=ea23ece4-599e-4a40-aa5f-b24744ac24b9',
						type : 'POST',
						dataType : 'json',
						data : param,
	...
	function openPopupReview(code, orderNumber, color, pPrice, imageUrl, bName,
			pName, categoryCode, returnType) {
		// 상품평 초기화
		reviewReset();
				...
				.ajax({
					...
					success : function(data) {
						...
						$('#photoAccumulationRightYn').val(
								data.photoAccumulationRightYn);
						if (data.orderProductReviewList.length > 0) {
							$('.select_options_wrap').hide(); // 색상, 사이즈 박스
							var productOptions = '';
							$
									.each(
											data.orderProductReviewList,
											function(idx, productOption) {
												if (orderNumber == productOption[1]
														&& style == productOption[3]
						...
						if (prodCode.indexOf('_') > -1) {
							...
							prodSize = prodCode.substring(tmpPnt + 1,
									prodCode.length);
							...
							if (prodCode.indexOf('_') > -1) {
								...
								$(".review_header_wrapper").css("margin-top",
										"16px");
							}
						}
	...
	function deleteReviewFile(li, idx) {
		...
		if ($('#productId').val() != '' && $('#productId').val() != null) {
			if (li.attr("id") != "" && li.attr("id") != null) {
				$("#reviewFileCd" + (li.attr("id") == "0" ? "" : li.attr("id")))
						.val('');
			}
		}
	}
	...
	function reviewWriteHtml(categoryCode, returnType) {
		...
		if (checkCategoryType(categoryCode) == "top"
				|| checkCategoryType(categoryCode) == "bottoms"
				|| checkCategoryType(categoryCode) == "cosmetic"
				|| checkCategoryType(categoryCode) == "perfume") {
		...
		if (checkCategoryType(categoryCode) == "cosmetic"
				|| checkCategoryType(categoryCode) == "perfume") {
		...
		if (checkCategoryType(categoryCode) == "cosmetic"
				|| checkCategoryType(categoryCode) == "perfume") {
			reviewhtml += "                         <tr class='radio_btn_1905'>";
			reviewhtml += "                             <th scope='row' class='th_space' id='thSkinType'>"
					+ reqd + "피부 타입</th>";
			reviewhtml += "                             <td colspan='3'>";
		...
		$(document).on(
				...
				"#reviewHeadline",
				function() {
					$(".write_star_wrap").next().next().next().find("th").css(
							"color", "black");
					$(".write_star_wrap").next().next().next().find("th").css(
							"font-weight", "normal");
				...
				.click(
						function() {
							...
							if ($('#orderWriteChk').val() == 'Y') { // 상품평쓰기 화면에서 색상이 보일때만 체크
								if ($('.select_options_wrap .color_chip li a')
										.hasClass('on') == false) {
									var la = new layerAlert('색상을 선택해 주세요.');
									//$('.select_options_wrap .color_chip li a').focus();
									$('.select_colors p').css("color",
											"#ff0000");
									$('.select_colors p').css("font-weight",
											"bold");
									return false;
								}
							}
							...
							if (checkCategory == "top"
								...
								if ($("#enjoySize").val() == null
										|| $.trim($("#enjoySize").val()) == '') {
									$("#thEnjoySize").css("color", "#ff0000");
									$("#thEnjoySize").css("font-weight",
											"normal");
									$("#enjoySize").focus();
								...
								if ($('input[name=bodyType]:checked').length < 1) {
									...
									$("#thBodyType").css("font-weight",
											"normal");
									$("#thBodyType").focus();
									var la = new layerAlert('체형을 선택해주세요.');

									$('input[name=bodyType]').on(
											'click',
											function() {
												$("#thBodyType").css("color",
														"black");
												$("#thBodyType")
														.css("font-weight",
																"normal");
								...
								if (checkCategory == "top") {
									if ($("input[name=shoulderWidth]:checked").length < 1) {
										var la = new layerAlert(
												'실착용 사이즈를 선택해주세요.');
										$(".write_star_wrap").next().find("th")
												.css("color", "#ff0000");
										$(".write_star_wrap").next().find("th")
												.css("font-weight", "bold");

										$("input[name=shoulderWidth]").click(
												function() {
													$(".write_star_wrap")
															.next().find("th")
															.css("color",
								...
								if ($("input[name=totalSize]:checked").length < 1) {
									...
									$("input[name=totalSize]").click(
											function() {
												$(".write_star_wrap").next()
														.find("th").css(
																"color",
																"black");
												$(".write_star_wrap").next()
								...
								if ($("input:radio[name=realProductColor]:checked").length < 1) {
									...
									$(".write_star_wrap").next().next().find(
											"th").css("color", "#ff0000");
									$(".write_star_wrap").next().next().find(
											"th").css("font-weight", "bold");
									la.confirmAction = function() {
										$("#customerReviewWriteDiv").attr(
												"tabindex", -1).focus();
									};

									$("input:radio[name=realProductColor]")
											.click(
													function() {
														$(".write_star_wrap")
																.next()
							...
							if (checkCategory == "cosmetic") {
								if ($('input[name=skinType]:checked').length < 1) {
									...
									$("#thSkinType").css("font-weight",
											"normal");
									$("#thSkinType").focus();
									var la = new layerAlert('피부 타입을 선택해주세요.');

									$('input[name=skinType]').on(
											'click',
											function() {
												$("#thSkinType").css("color",
														"black");
												$("#thSkinType")
														.css("font-weight",
																"normal");
								...
								if ($("input[name=spreadability]:checked").length < 1) {
									...
									$("input[name=spreadability]").click(
											function() {
												$(".write_star_wrap").next()
														.next().find("th").css(
																"color",
								...
								if ($("input[name=scent]:checked").length < 1) {
									...
									$(".write_star_wrap").next().next().next()
											.find("th").css("color", "#ff0000");
									$(".write_star_wrap").next().next().next()
											.find("th").css("font-weight",
													"bold");

									$("input[name=scent]").click(
											function() {
												$(".write_star_wrap").next()
														.next().next().find(
																"th").css(
								...
								if (headline.trim().length < 20
										|| headline.trim().length > 200) {
									var la = new layerAlert(
											'상품평 내용은 20~200자 사이입니다.');
									$(".write_star_wrap").next().next().next()
											.next().find("th").css("color",
													"#ff0000");
									$(".write_star_wrap").next().next().next()
							...
							var lc = new customLayerConfirm("상품평을 등록하시겠습니까?",
									"확인", "취소"); // 상품평을 등록하시겠습니까?, 확인, 취소
							lc.confirmAction = function() {
