...
<head>
	...
	<script type="text/javascript">
		...
		ACC.config.CSRFToken = "d528244a-936e-43d6-98ab-06fb4e4c85d1";
	...
	<script>
		$(document).ready(function () {
			...
			if (todayDate.getTime() >= startDate.getTime() && todayDate.getTime() <= endDate.getTime()) {
				...
				var referer = "https://www.thehandsome.com/ko/checkout/orderConfirmation";
						...
						$.ajax({
							...
							success: function (data) {
								...
								if (data != "11") {
									...
									innerHtml += '        <a href="javascript:setPlayLeaves(' + data +
										')">행운의 단풍잎 줍기!</a>';
									innerHtml += '    </div>';
									innerHtml +=
...
<body oncontextmenu='return false'>
	...
	<form id="CSRFForm" action="/ko/mypage/order/myorders" method="post">
		<div>
			<input type="hidden" name="CSRFToken" value="d528244a-936e-43d6-98ab-06fb4e4c85d1" />
		</div>
	</form><!-- headerWrap -->
	<div id="headerWrap">
				...
				function callChatbot(token, talkId) {
					if (token == null) {
						$.ajax({
							...
							data: {
								"CSRFToken": "d528244a-936e-43d6-98ab-06fb4e4c85d1"
							},
							async: false,
							success: function (data) {
								if (data.result == "SHOULD_LOGIN") {
									...
									"OFF-HOURS") { //일반 : CREW, FRIEND, 간편회원일경우, 평일 오전 9시 30분 ~ 오후 6시 30분만 사용가능
			...
			<script type="text/javascript">
				...
				$(document).ready(function () {
					...
					if (getCookie("ie8AlertHideYN") != "on") {
						if (navigator.appVersion.indexOf('MSIE 8.0') >= 0 || navigator.appVersion.indexOf('Trident/4.0') >=
							0) {
							var html =
						...
						for (var j = 0; j < arr.length; j++) {
							if (arr_date[i] != "") {
								if (arr[j].indexOf(arr_date[i]) > 0) {
									...
									$("#searchWord" + (i + 1)).next().attr("onclick", "deletecookie('HS_Seachwords" + (j +
										1) + "')");
								}
							}
						}
					...
					if ("PASS" != "") {
						...
						var chkPag = ["svCenterMainPage", "noticePage", "manToManInquiryPage", "faqPage",
							...
							"theClubInfoPage",
							"memberBenefitGuidePage", "pointGuidePage", "couponGuidePage", "paymentGuidePage",
							"asGuidePage"
						];
							...
							if (!$('.search_box_wrap').hasClass('on')) {
								...
								$(document).find('#styleSearchSlider').flexslider({
									...
									after: function (slider) {
										$("#styleSearchSlider .slides li").not(".clone").each(
											function (idx) {
												if ($(this).hasClass("flex-active-slide")) {
													$(".ss_txt_list li a").removeClass(
													"on");
													$(".ss_txt_list li:eq(" + idx + ") a")
														.addClass('on');
												}
											});
									}
                        ...
                        $(".autocomplete_right").height(atctxtHeight);
									...
									$('.autocomplete_txt li a').each(function () {
										if (!$(this).hasClass('brand_main')) {
											...
											if ($(this).html().indexOf($('#searchBox')
												.val()) > -1) {
								...
								if (!$('#query').val() == "") {
									...
									$('.search_box_wrap.on .search_box input[type="search"]').stop()
										.animate({
											width: '461px'
										}, 'fast');
								} else {
							...
							<div class="search_tab">
								...
								<form id="searchStyle" name="searchStyle" action="/ko/hssearch/searchCount"
									...
									<input type="hidden" id="CSRFToken" name="CSRFToken"
										value="d528244a-936e-43d6-98ab-06fb4e4c85d1" />
			...
			<div class="sub_cnts">
				...
				<form id="orderSearchForm" action="/ko/mypage/order/myorderlist">
					...
					<div class="search_wrap">
						...
						<ul>
							...
							<li class="space">
								<p class="bul_sty01"><label for="id_sel">검색구분
										<!-- 검색구분 --></label></p>
					...
					<table class="tbl_ltype review_betterment1905">
						...
						<tbody id="listBody">
							<tr class="al_middle">
								...
								<td>
									<div class="pt_list_all"> <a href="/ko/p/YS2D1ACK001WHD_BK_FR"><img
										...
										<div class="tlt_wrap"> <a href="/ko/p/YS2D1ACK001WHD_BK_FR" class="basket_tlt">
											...
											<p name="__p_YS2D1ACK001WHD_BK_FR" class="guide_comment2"
												style="display: none;">해당 상품은 전국 매장을 통해 수급하여 배송 될 예정입니다. 불량 등의 이유로 수급이
												어려울 경우, 취소될 수 있습니다. (평일 기준 5일 이상 소요 예정)</p>
										</div>
									</div>
								</td>
								<td><del>1</del>0</td>
							...
							<tr class="al_middle">
								<td>
									<div class="pt_list_all"> <a href="/ko/p/YN2C9ACK491W_BG_FR"><img
							...
							<tr class="al_middle">
								...
								<td>
									<div class="pt_list_all"> <a href="/ko/p/YN2C9ACK492W_IV_FR"><img
					...
					<dl class="gd_list com pl0">
						...
						<dd>
							<ul class="bul_sty01_li">
								<li>한섬마일리지는 배송 완료 후 10일 후 지급 됩니다.
									<!-- 한섬마일리지는 배송 완료 후 10일 후 지급 됩니다. -->
								</li>
								<li>반품신청은 배송완료 후 7일까지 가능 합니다.
									<!-- 반품신청은 배송완료 후 7일까지 가능 합니다. -->
								</li>
								<li>배송준비중 상태에서는 주문의 취소/변경이나 주소의 변경이 불가능 합니다.
									<!-- 배송준비중 상태에서는 주문의 취소/변경이나 주소의 변경이 불가능 합니다. -->
								</li>
								<li>주문 시 사용한 기프트 카드, 한섬마일리지는 모두 동일 수단으로 환불됩니다.
									<!-- 주문 시 사용한 기프트 카드 , 한섬마일리지는 모두 동일 수단으로 환불됩니다. -->
								</li>
								<li>한섬마일리지는 정상 매장 기준 5%, 아울렛 1% 적립됩니다. (세일상품 제외)
									<!-- 한섬마일리지는 정상 매장 기준 5%, 아울렛 1% 적립됩니다. (세일상품 제외) -->
								</li>
								<li>10%를 초과하여 할인하는 상품은 마일리지 적립 대상에서 제외됩니다.
									<!-- 10%를 초과하여 할인하는 상품은 마일리지 적립 대상에서 제외됩니다. -->
								</li>
								<li>주문완료 상태까지는 배송지 수정이 가능 합니다.
									<!-- 주문완료 상태까지는 배송지 수정이 가능 합니다. -->
								</li>
								<li>가상계좌 결제 시 입금대기 상태에서 4시간 내 입금하지 않으면 자동으로 주문이 취소 됩니다.
									<!-- 가상계좌 결제 시 입금대기 상태에서 24시간 내 입금하지 않으면 자동으로 주문이 취소 됩니다. -->
								</li>
								<li>상세보기 화면에서 증빙서류의 출력이 가능 합니다.
									<!-- 상세보기 화면에서 증빙서류의 출력이 가능 합니다. -->
								</li>
							</ul>
						</dd>
					</dl>
				...
				<div class="gd_wrap">
					<div class="delivery_step">
						<p class="title">주문/배송프로세스
							<!-- 주문 / 배송 프로세스 -->
						</p>
						<ol>
							<li>
								<p class="step"><span>STEP 1</span> 1. 입금대기중
									<!-- 입금 대기중 -->
								</p>
								<p class="txt">가상계좌를 선택 하셨다면 결제를 완료해 주세요.
									<!-- 가상계좌를 선택 하셨다면<br>결제를 완료해 주세요. -->
								</p>
							</li>
							<li>
								<p class="step"><span>STEP 2</span> 2. 주문완료
									<!-- 주문완료 -->
								</p>
								<p class="txt">주문정보를 확인하고 있습니다.
									<!-- 주문정보를 확인 하고<br>있습니다. -->
								</p>
							</li>
							<li>
								<p class="step"><span>STEP 3</span> 3. 배송준비중
									<!-- 배송 준비중 -->
								</p>
								<p class="txt">상품의 배송을 준비하고 있습니다.
									<!-- 상품의 배송을 준비하고<br>있습니다. -->
								</p>
							</li>
							<li>
								<p class="step"><span>STEP 4</span> 4. 배송중
									<!-- 배송중 -->
								</p>
								<p class="txt">물품이 발송되어 고객님께 배송 중 입니다.
									<!-- 물품이 발송되어 고객님께<br>배송 중 입니다. -->
								</p>
							</li>
							<li>
								<p class="step"><span>STEP 5</span> 5. 배송완료
									<!-- 배송완료 -->
								</p>
								<p class="txt">배송이 완료 되었습니다.
									<!-- 배송이 완료 되었습니다.-->
								</p>
							</li>
						</ol>
...
<div class="popwrap w_type_4" id="addrModPop" style="display:none;">
							...
							<div class="input_sumtxt_box">
								...
								<div class="delivery_comt">
									<ul>
										...
										<li class="deliveryMsg"><a href="#;" data-default="자유 출입 가능"
	...
	<!-- btn_close -->
	<a href="#" class="btn_close"><img src="http://cdn.thehandsome.com/_ui/desktop/common/images/popup/ico_close.png"
			alt="닫기"></a>
	<!-- //btn_close -->
</div>
<!--//layer popup-->
			...
			<table class="tbl_wtype1">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<input value="" title="mypage.delivery.contactname.english" name="lastname" id="name"
					...
					<tr>
						...
						<td>
							<input value="" title="mypage.delivery.contactname.local" name="localLastName"
