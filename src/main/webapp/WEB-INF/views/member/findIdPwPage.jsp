...
<script type="text/javascript">
		...
		var ACC = { config: {} };
			...
			ACC.config.CSRFToken = "e268be3c-4fdd-46d9-af59-52dbc226ad3a";
...
// 								html.push('<li><a href="http://www.thehyundai.com/front/dpm/myShopHome.thd?bfp=Ref_Handsome">트렌드샵</a></li>');
	...
	<form id="CSRFForm" action="/ko/member/findIdPwPage" method="post"><div>
<input type="hidden" name="CSRFToken" value="e268be3c-4fdd-46d9-af59-52dbc226ad3a" />
</div></form><!-- headerWrap -->
		<div id="headerWrap" >
...
function callChatbot(token, talkId) {
    if(token == null) {
        $.ajax({
            ...
            data: {"CSRFToken":"e268be3c-4fdd-46d9-af59-52dbc226ad3a"},
            async : false,
            success:function(data){
...
$(document).ready(function() {
    ...
    if("" != ""){
	    ...
	    if ((vipPassCheck == "Y" || timePassCheck == "Y") && "hsFindIdPwPage" == "main201903Page") {
	        $('#chatbotMain').show(); 
	    }else{
	        $('#chatbotMain').hide(); 
	    }
                    ...
                    <div class="data_react">
                        ...
                        <div class="search_tab">
                            ...
                            <form id="searchStyle" name="searchStyle" action="/ko/hssearch/searchCount" method="post" onsubmit="return false;">
                                ...
                                <input type="hidden" name="collection" value=""/>
                                <input type="hidden" id="CSRFToken" name="CSRFToken" value="e268be3c-4fdd-46d9-af59-52dbc226ad3a" />
...
$(document).ready(function(){
	...
	{
		$("#sendMsg").text("email.address");	//다국어 : 이메일주소
		tabChange("sendEmail");
	}
	
	
	$("#AuthBtn_Email").click(function(){
		paramAuth = $("input:radio[name='findEmail']:checked").val();
		AuthSelect(paramAuth);
	});
	
	
	$("#AuthBtn_PW").click(function(){
		if($("#idVelidation").val() == "Y")
		{
			sendName = $("#sendName").val();
			userid = $("#user_id").val();
			paramAuth = $("input:radio[name='findPW']:checked").val();
			AuthSelect(paramAuth);
		}
		else
		{
	...
	var AuthSelect = function(selectVal){		
		if(selectVal == "email_M")
		{
			$("#authName").val(selectVal);
			window.open('/ko/member/phoneauth?fr=email_M', 'popupChk', 'width=500, height=550, top=100, left=100, fullscreen=no, menubar=no, status=no, toolbar=no, titlebar=yes, location=no, scrollbar=no');
		}
		else if(selectVal == "pw_M")
		{
			$("#sendName").val(sendName);
			$("#authName").val(selectVal);
			window.open('/ko/member/phoneauth?fr=pw_M&sendName=' + sendName + "&userid=" + userid, 'popupChk', 'width=500, height=550, top=100, left=100, fullscreen=no, menubar=no, status=no, toolbar=no, titlebar=yes, location=no, scrollbar=no');
		}
		else if(selectVal == "email_I")
		{
			$("#authName").val(selectVal);
			window.open('/ko/member/ipinauth?fr=email_I', 'popupChk', 'width=500, height=550, top=100, left=100, fullscreen=no, menubar=no, status=no, toolbar=no, titlebar=yes, location=no, scrollbar=no');
		}
		else
		{
			$("#sendName").val(sendName);
			$("#authName").val(selectVal);
			window.open('/ko/member/ipinauth?fr=pw_I&sendName=' + sendName + "&userid=" + userid, 'popupChk', 'width=500, height=550, top=100, left=100, fullscreen=no, menubar=no, status=no, toolbar=no, titlebar=yes, location=no, scrollbar=no');
	...
	if(customerName != ""){
		...
		{
			$("input:radio[name='findEmail']:radio[value=" + authName + "]").prop("checked",true);
			$("#result_id").show();
		}
		else
		{
			$("input:radio[name='findPW']:radio[value=" + authName + "]").prop("checked",true);
			$("#result_pw").show();
		}
	}
	...
	$("#user_id").blur(function(){
		$.ajax({
			type:"GET",
			url:"/ko/member/isdupluid?uid="+$("#user_id").val(),
    ...
    $("#searchIDBtn").click(function(){
        var birthday = $("#selYear").val() + $("#selMonth").val() + $("#selDay").val();
        var vc = new ValidationCheck(); 
        vc.checkIdList = ['iUserName', 'selDay', 'selMonth', 'selYear'];
        if(vc.isValid()){
            $("#iBirthday").val(birthday);
            $("#easyIdSearchForm").attr("action", "/ko/member/easyIdFind");
            $("#easyIdSearchForm").submit();   
        }
    });
    ...
    $("#searchPWBtn").click(function(){
        var vc = new ValidationCheck(); 
        vc.checkIdList = ['pUserName', 'pUserEmail'];
        
        if(vc.isValid()){
            $("#searchPWBtn").attr("disabled", true);
            var formData = $("#easyPwSearch").serialize();
            $.ajax({
	            type:"GET",
	            url:"/ko/member/searcheasyjoinpw",
	            data : formData,
	            cache : false,
	            success:function(data){
	                if(data.resultCode == "S"){
	                    layerAlert("회원가입시 입력하신 이메일로 임시비밀번호가 발송되었습니다.");
	                }else{
...
<form id="resultSend" action="/ko/member/findIdPwPage" method="POST"><input type="hidden" name="idVelidation" id="idVelidation" value="" />
...
<div>
<input type="hidden" name="CSRFToken" value="e268be3c-4fdd-46d9-af59-52dbc226ad3a" />
</div></form><div id="bodyWrap" class="login">
	<!--title-->
	<h3 class="cnts_title"><span>아이디/비밀번호 찾기</span></h3>
	<!--//title-->
	<!--sub container-->
	<div class="sub_container">
			<fieldset>
				<legend>아이디/비밀번호 찾기 입력항목</legend>
				<!-- tab추가 2017-05-11-->
                <div class="tabType1">
                    <a href="javascript:isShow('');" id="h1">한섬회원</a>
                    <a href="javascript:isShow('e');" id="e1">간편회원</a>
                </div>
                <!--// tab추가 2017-05-11-->
				<div class="login_wrap box_type_1" id="handsomeM">
					<div class="inner_box">
						<div class="title_wrap mb26">
							<h4>아이디 찾기</h4>
							<p class="txt">본인 명의 휴대폰 번호와 회원가입 시 아이핀 인증으로 가입한 경우 <br />본인여부를 확인 합니다.</p>
						</div>
						<div class="mobile_ipin clearfix">
							<div class="mobile">
								<p><input type="radio" name="findEmail" id="email_mobile" value="email_M" checked/><label for="email_mobile">휴대폰 인증</label></p>
							</div>
							<div class="ipin">
								<p><input type="radio" name="findEmail" id="email_ipin" value = "email_I"/><label for="email_ipin">아이핀(I-PIN) 인증</label></p>
							</div>
						</div>
						<div class="btnwrap">
							<a href="javascript:void(0);" class="btn wt mr0" id="AuthBtn_Email">인증하기</a>
							<!-- <input type="button" class="btn wt mr0" value="인증하기" /> -->
						</div>
						<div class="login_result" id="result_id" style="display:none;">
							<p>고객님의 ID는 <strong class="ft_point01"></strong> 입니다.</p>
							<div class="btnwrap mt0"><a href="/ko/member/login" class="btn gray_s mr0">로그인</a></div>
						</div>
					</div>
					<div class="inner_box step2">
						...
						<div class="tab_a m2">
							<ul>
								<li><a href="javascript:tabChange('sendEmail');" class="active" id="sendEmail">등록된 이메일로 받기</a></li>
								<li><a href="javascript:tabChange('sendSms');" class="" id="sendSMS">등록된 핸드폰으로 받기</a></li>
							</ul>
						</div>
						<!--// tab -->
						<div class="id_input">
						...
						<p class="login_err_txt">아이디를 입력하세요.</p>
						<div class="mobile_ipin clearfix">
							<div class="mobile">
								<p><input type="radio" name="findPW" id="pw_mobile" value="pw_M" checked/><label for="pw_mobile">휴대폰 인증</label></p>
							</div>
							<div class="ipin">
								<p><input type="radio" name="findPW" id="pw_ipin" value="pw_I"/><label for="pw_ipin">아이핀(I-PIN) 인증</label></p>
							</div>
						</div>
						<div class="btnwrap">
							<a href="javascript:void(0);" class="btn wt mr0" id="AuthBtn_PW">인증하기</a>
						</div>
						<div class="login_result" id="result_pw" style="display:none;"><span class="guide_comment" id="emailMsg" ></span>
							<p></p>
						</div>
					</div>
				</div>
				<!-- 탭 내용 추가 2017-05-11-->
                <div class="login_wrap box_type_1 type3" id="easyM" style="display:none">
                    <div class="inner_box">
                        <div class="title_wrap">
                            <h4>아이디 찾기</h4>
                            <p class="txt">회원가입 시 입력한 이름, 생년월일을 입력해주세요.</p>
                        </div>
                        <form id="easyIdSearchForm" name="easyIdSearchForm" action="/ko/member/findIdPwPage" method="POST"><div class="id_input">
                            <p class="mb10">
                                <label for="user_name">이름</label>
                                <input id="iUserName" name="iUserName" placeholder="이름을 입력하세요." type="text" />
                                <input id="iBirthday" name="iBirthday" type="hidden" />
                            </p>
                            <p>
                               <label for="birthday">생년월일</label>
                               <select style="width:105px" id="selYear">
                                    <option value="">년</option>
                               </select> 
                               <select style="width:105px" id="selMonth">
                                ...
                                <select style="width:105px" id="selDay">
                        ...
                        <div>
<input type="hidden" name="CSRFToken" value="e268be3c-4fdd-46d9-af59-52dbc226ad3a" />
</div></form><div class="btnwrap">
                            <input type="button" class="btn wt" id="searchIDBtn" value="확인" />
                        </div>
                    </div>
                    <div class="inner_box step2">
                        <div class="title_wrap">
                            <h4>비밀번호 찾기</h4>
                            <p class="txt">간편회원 가입 시, 등록한 이름과 정보 수신용 이메일을 입력해주세요.</p>
                        </div>
                        ...
                        <div class="id_input">
                            <p class="mb10">
                                ...
                                <input id="pUserName" name="pUserName" type="text" placeholder="이름을 입력하세요." />
                            </p>
                            <p>
                                <label for="pUserEmail">이메일</label>
                                <input id="pUserEmail" name="pUserEmail" type="text" placeholder="정보수신용 이메일을 입력하세요."  />
                            </p>
                        </div>
                        </form>
                        <p class="login_err_txt" id="pwMsg"></p> 
                        <div class="btnwrap">
                            <input type="button" class="btn wt" id="searchPWBtn" value="확인" />
                        </div>
                    </div>
                </div>
                <!--// 탭 내용 추가 2017-05-11-->
				<div class="btm_notice">
					<ul class="bul_sty01_li"> 
						<li>H.Point 멤버십 회원일 경우 <a href="https://www.h-point.co.kr/cu/config/findCustId.nhd?prtnrId=D080&chnnlId=1705" target="_blank"><em class="underline">아이디 찾기 ></em></a> / <a href="https://www.h-point.co.kr/cu/config/findCustPwd.nhd" target="_blank"><em class="underline">비밀번호 찾기 ></em></a> 중 해당 버튼 클릭 후, H.Point 멤버십 웹사이트로 이동하여 진행해주세요. </li>
						<li>입력하신 정보는 본인확인을 위해 <em class="ft_point01">서울신용평가정보(주)</em>에 제공되며, 본인확인 용도 외에 사용되거나 저장되지 않습니다.</li>
						<li>임시비밀번호로 로그인 후 비밀번호를 변경해주세요. <a href="/ko/mypage/personInfomationChange"><em class="underline">마이페이지 > 나의 정보관리 > 개인정보변경</em></a>에서 변경 하실 수 있습니다.</li>
					</ul>
				</div>
			</fieldset>
	</div>
